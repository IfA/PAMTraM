...
public class SimpleToExternalComplexAttributeMappingAction extends Action {
	
	SimpleAttributeMapping oldMapping;


	public SimpleToExternalComplexAttributeMappingAction(SimpleAttributeMapping m) {
		super("convert to external ComplexAttributeMapping");
		oldMapping=m;
	}
	...
	public void run() {
		...
		ComplexAttributeMappingExternalSourceElement source= (ComplexAttributeMappingExternalSourceElement) ePackage.getEFactoryInstance().create((EClass) ePackage.getEClassifier("ComplexAttributeMappingExternalSourceElement"));
		...
		if(oldMapping.getModifier() != null){
			source.eSet(source.eClass().getEStructuralFeature(MappingPackage.COMPLEX_ATTRIBUTE_MAPPING_EXTERNAL_SOURCE_ELEMENT__MODIFIER), oldMapping.getModifier());
		}
