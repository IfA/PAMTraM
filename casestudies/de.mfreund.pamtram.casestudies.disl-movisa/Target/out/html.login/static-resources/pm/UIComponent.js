 ...
 */
UIComponent.prototype.setRepresentationAttributes = function(repObj, animation){
	var sizeSet = false;
	var accessible = false;
	var valid = false;

	// has to be set so that padding does not add to the height/width of the object
	this.htmlContainer.css("-moz-box-sizing", "border-box");
	this.htmlContainer.css("-webkit-box-sizing", "border-box");
	this.htmlContainer.css("box-sizing", "border-box");
...
UIComponent.prototype.setCssObjectProperty = function(repObj, key, 
	...
	var cssProperty = mapping[key][movisaProperty]["Equivalent"];
	...
	}//	} else if(movisaProperty === "Background") {
//		
//		//set the styling for the HTML-Element
//		if(mapping[key][movisaProperty]["connectedToElement"]){
//			//this.htmlElement.prop('style').removeProperty('background-color')
//			this.htmlElement.css(cssProperty, null);
//		}
//		
//		//set the styling for the HTML-Container
//		if(mapping[key][movisaProperty]["connectedToContainer"]){
//			this.htmlContainer.css(cssProperty, null);
//		}
//	}
};
	...
	switch (flashObj['FlashType']) {
		case 'BACKGROUND':
			...
			flashObj['FlashResetColor'] = 
				this.representation["Color"]["Background"] || null;
			break;
		case 'FOREGROUND':
			flashObj['FlashRepresentationPropertyOn'] = { 
				"Color": { 
			...
			flashObj['FlashResetColor'] = 
				this.representation["Color"]["Foreground"] || null;
			break;
		case 'BORDER':
			flashObj['FlashRepresentationPropertyOn'] = { 
				"Border": { 
			...
			flashObj['FlashResetColor'] = 
				this.representation["Border"]["LineColor"] || null;
			break;
		default:
			return;
	}
...
UIComponent.prototype.doFlash = function(flashObj){
	...
	if (typeof flashObj['ActualState'] !== "boolean") {
		return;
	}
	
	// If actual state is on (true), then shut of.
	if (flashObj['ActualState']) {
		this.setRepresentationAttributes(
				flashObj['FlashRepresentationPropertyOff'], true);
	}
	// If actual state is off (false), then switch on.
	else {
		this.setRepresentationAttributes(
				flashObj['FlashRepresentationPropertyOn'], true);
	}
	// Set actual state.
...
UIComponent.prototype.representationAnimationFactory = function(propertyToAnimateName, record){
	if(propertyToAnimateName !== undefined){
		return function(){
				...
				//var repObj = $.extend(true, {}, this.representation, record[propertyToAnimateName]);
			var repObj = $.extend(
					$.extend(true, {}, this.representation),
					$.extend(true, {}, record[propertyToAnimateName])
				);
				/*var ar = record[propertyToAnimateName];
				for (var key in ar) {
