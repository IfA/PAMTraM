...
public class MetaModelSectionGenerator {
		...
		for(EReference eContainment : eAllContainments) {
			...
			for(EObject eObject : contained) {
				...
				if(existing != null && this.dangling.contains(existing)) {
					...
					((EList<Class<?, ?, ?, ?>>) containmentReference.getValue()).add(this.createMetaModelSection(eObject));
				}
			}
	...
	public LinkedList<Section<?, ?, ?, ?>> mergeDuplicates(LinkedList<Section<?, ?, ?, ?>> created) {
		...
		(EList<Section<?, ?, ?, ?>>) (this.sectionType == SectionType.SOURCE ? this.pamtram.getSourceSections() : this.pamtram.getTargetSections());
	...
	private boolean compare(Section<?, ?, ?, ?> createdSection, EList<Section<?, ?, ?, ?>> pamtramSections) {
		...
		if(!potentialMatches.isEmpty()) {
			...
			Collection<Setting> crossReferences = EcoreUtil.UsageCrossReferencer.find(createdSection, this.created.values());
