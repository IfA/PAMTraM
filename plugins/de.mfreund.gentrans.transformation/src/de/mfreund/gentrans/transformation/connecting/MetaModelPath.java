...
public class MetaModelPath {
	 ...
	 *         enough).
	 */
	public List<EObjectWrapper> instantiate(final EObject rootObject,
			final Collection<EObjectWrapper> objectsToConnect) {

		return this.instantiateMissingPath(this.getInvertedPathElementList(), rootObject,
	 ...
	 */
	@SuppressWarnings("unchecked")
	private List<EObjectWrapper> instantiateMissingPath(final LinkedList<EObject> invertedPath,
			final EObject rootObject, List<EObjectWrapper> objectsToConnect) {
		...
		if (pathCopy.size() > 1) {
			...
			if (ref.getUpperBound() == 1) {// only one target instance allowed,
				...
				if (targetInst == null) {
					...
					this.targetSectionRegistry.addClassInstance(inst);
				...
				while (!objectsToConnect.isEmpty()) {
					...
					this.targetSectionRegistry.addClassInstance(newTarget.getLast());
			...
			if (ref.getUpperBound() == 1) {
				if (targetInst != null) {
					...
					rootObject.eSet(ref, objectsToConnect.remove(0).getEObject());
