...
/**
 * This base class represents a map that allows to collect hint values for mapping hints that are based on attribute
 * values.
 * <p />
 * Thereby, the stored hint values are associated with the <em>mapping hint</em>, the source element of the mapping hint
 ...
 */
public class AttributeBasedHintValueMap<K extends EObject, S extends EObject>
		extends HintValueMap<K, Map<S, AttributeValueRepresentation>> {
	...
	public void addHintValues(HintValueMap<K, Map<S, AttributeValueRepresentation>> valueMap) {
		...
		for (final K h : valueMap.keySet()) {
			...
			for (final SourceSectionClass c : valueMap.get(h).keySet()) {
				...
				if (valueMap.get(h).get(c).isEmpty() || valueMap.get(h).get(c).getFirst().isEmpty()) {
					continue;
				}

				/*
				 * Finally, add the hint values. However, if the partial values stored in the 'valueMap' represent
				 * different 'S' than those already present in 'this', we combine the values instead. Thus, we ensure
				 * that complex hint values can be created from source attributes at different layers of the containment
				 * tree.
				 */
					...
					if (!combine) {
						if (this.get(h).get(c).isEmpty()) {
							...
							for (Map<S, AttributeValueRepresentation> newValue : newValues) {
								for (java.util.Map.Entry<S, AttributeValueRepresentation> oldValueEntry : oldValue
										.entrySet()) {
						...
						for (Map<S, AttributeValueRepresentation> element : this.get(h).get(c)) {
							for (java.util.Map.Entry<S, AttributeValueRepresentation> entry : valueMap.get(h).get(c)
									.getFirst().entrySet()) {
