...
public class GlobalAttributeValueExtractor extends ValueExtractor {
	...
	public Map<GlobalAttribute, String> extractGlobalAttributeValues(
		...
		// Collect the GlobalAttributes that are modeled for each MappingModels and the associated
		// MatchedSectionDescriptors
		//
		Map<GlobalAttribute, List<MatchedSectionDescriptor>> sourceSectionByGlobalAttribute = mappingModels
				.parallelStream().flatMap(m -> m.getGlobalAttributes().parallelStream())
				.collect(Collectors.toMap(i -> i, i -> matchingResult.get(i.getSource().getContainingSection())));
